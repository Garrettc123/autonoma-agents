...
- **Application memories are rows, addressed by slug** - `application_memory` holds owner-authored knowledge about an application (`slug`, `title`, `description`, free-form `content`, `enabled`), unique on `(application_id, slug)`. The slug is the agent-facing handle: set when the row is created and never rewritten, so a title edit cannot break a slug an agent has already read. Only `enabled` rows are visible to the pipeline. Rows are written by hand with the `memories:upsert` script from a directory of one frontmatter file per memory (`apps/api/README.md`); the index agents see is rendered by `@autonoma/types`.
