 ...
 * to that run's test-result page.
 *
 ...
 * `removedInvalidTests` uses) so the first row seen per test is its latest run - its finding `id`, which the row's
 * test-result page link keys on - then returned slug-ordered for a stable within-tier order downstream.
 */
