...
- plan_mismatch -> the platform is HEALTHY and the described behavior IS reachable, but the run did NOT reach or exercise it because the WRITTEN STEPS do not match the app (a renamed, removed or relocated control, a redesigned flow, a stale route, a behavior change the plan predates). A step the plan got RIGHT that the agent skipped or misfired (a truncated trace that never attempted the written navigation) is an engine/agent stall -> engine_artifact, NOT plan_mismatch. Missing seeded data is NEVER plan_mismatch (no step edit creates data - that is scenario_issue). Emit the COMPLETE corrected plan in suggestedTestUpdate and explain the mismatch and rewrite in planMismatchNote.
...
plan_mismatch - platform healthy, description reachable but NOT reached because a written step is wrong.
invalid_test - no plan can satisfy the description on a healthy platform; remove.`;

/**
 * The verdict rules, rendered as the closing section of the user prompt, so the model selects a verdict tool
...
function buildVerdictRules(): string {
    return "When your investigation is complete, call exactly one verdict tool. The system prompt's INVARIANTS and six-owner decision determine which tool to choose; that tool's description and field schemas define the evidence and category-specific result it requires. Fill it with this loop's tool results still in scope.";
}
 ...
 * the verdict rules the selected terminal tool is filled against. One prompt, so the model's tool results are still in
...
function buildScanSections(scans: ProbeScans): string[] {
    return [
        ...
        ...scanSection(
            ...
            scans.fidelityScan,
            "If the run DIVERGED from the plan, it never actually exercised the intended behaviour - the 'failure' is then most likely the test/plan not matching the UI (plan_mismatch), NOT an app defect. A client_bug verdict REQUIRES that the run faithfully reached and exercised the behaviour under test.",
        ),
        ...scanSection(
 ...
 * model to treat the text above it as observed fact, so emitting it
